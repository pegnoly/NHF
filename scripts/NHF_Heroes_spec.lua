...
NHF_Mirabel_lvl_temp = 0;
NHF_Mirabel_temp_week = 0;
function  error_NHF_Mirabel_F ()
	print("error:NHF_Mirabel_F");
end;
AddEvent(new_day_events, 'NHF_mirabel_new_day',
function(hero, day)
  if hero == 'Mirabel' and IsHeroAlive('Mirabel') then
    errorHook(error_NHF_Mirabel_F);
    if NHF_Mirabel_temp_week ~= (ceil(GetDate(ABSOLUTE_DAY)/7)) then
  	  NHF_Mirabel_temp_week = (ceil(GetDate(ABSOLUTE_DAY)/7));
  	  local prirost = ceil(GetHeroLevel("Mirabel")*NHF_Mirabel_percent);
  	  if IsObjectExists("Mirabel") == true then
  		if IsHeroInBoat("Mirabel") == true then
  		   prirost = ceil(GetHeroLevel("Mirabel")*NHF_Mirabel_sheep_percent);
  		end;
  	  end;
  	  local temp_name = NHF_tempName_F("Mirabel");
  	  if (GetHeroLevel("Mirabel") >= NHF_Mirabel_lvl_temp) then
  		NHF_Mirabel_lvl_temp = GetHeroLevel("Mirabel");
  		AddHeroCreatures("Mirabel", CREATURE_PIRATE, prirost);
  		startThread(NHF_ShowFlyMessage,{NHF_Mirabel_textPath.."message.txt";p=prirost}, temp_name,5);
  		--startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅпїЅ: +"..prirost),temp_name , GetObjectOwner(temp_name),5);
  	  end;
    end;
  end
end)

--NHF---------------пїЅпїЅпїЅпїЅпїЅпїЅпїЅ-------------------
NHF_Oktavio_textPath = "/MapObjects/Haven/NewHeroes/Oktavio/";
NHF_Oktavio_week_temp = 0;
NHF_Oktavio_SP_bonus = 0;
NHF_Oktavio_town_prirost_mass = {};
for i,town in GetObjectNamesByType('TOWN')  do
	NHF_Oktavio_town_prirost_mass[i] = 0;
end;
function  error_NHF_Oktavio_F ()
	print("error:NHF_Oktavio_F");
end;

AddEvent(new_day_events, 'NHF_oktavio_new_day',
function(hero, day)
  if hero == 'Oktavio' and IsHeroAlive("Oktavio") then
    errorHook(error_NHF_Oktavio_F);
    local temp_name = NHF_tempName_F("Oktavio");
  	for i,town in GetPlayerTowns(GetObjectOwner('Oktavio'))  do
  		if GetObjectOwner(town) ~= 0 then
  			if  ((GetTownRace(town) == TOWN_HEAVEN) and ((ceil(GetDate(ABSOLUTE_DAY)/7)) ~= NHF_Oktavio_town_prirost_mass[i]) and (GetPlayerTeam(GetObjectOwner(town)) ==  GetPlayerTeam(GetObjectOwner(temp_name)))) then
  				if GetTownBuildingLevel(town, TOWN_BUILDING_DWELLING_5) ~= 0 then
  				   NHF_Oktavio_town_prirost_mass[i] = (ceil(GetDate(ABSOLUTE_DAY)/7));
  				   local prirost = ceil(GetHeroLevel("Oktavio")*NHF_Oktavio_percent);
  				   SetObjectDwellingCreatures(town, CREATURE_PRIEST, GetObjectDwellingCreatures(town,CREATURE_PRIEST) + prirost);
  				   startThread(NHF_ShowFlyMessage,{NHF_Oktavio_textPath.."message0.txt";p=prirost}, town,5);
  				   --startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅ +"..prirost),town ,5);
  				end;
  			end;
  		end;
  	end;
  	if  NHF_Oktavio_week_temp ~= (ceil(GetDate(ABSOLUTE_DAY)/7)) then
  		NHF_Oktavio_week_temp =  (ceil(GetDate(ABSOLUTE_DAY)/7));
  		local prirost = ceil(GetHeroLevel("Oktavio")*NHF_Oktavio_percent);
  		if prirost > 0 then
  			NHF_creatures_prirost_F("Oktavio",prirost,NHF_mass_creatures.Haven.L5);
  		end;
  	end;
  	local NHF_Oktavio_SP_bonus_temp = ceil((GetHeroCreatures("Oktavio",CREATURE_PRIEST)+GetHeroCreatures("Oktavio",CREATURE_CLERIC)+GetHeroCreatures("Oktavio",CREATURE_ZEALOT))/NHF_Oktavio_percent_SP);
  	if NHF_Oktavio_SP_bonus ~=  NHF_Oktavio_SP_bonus_temp then
  		local delta = NHF_Oktavio_SP_bonus_temp - NHF_Oktavio_SP_bonus;
  		ChangeHeroStat("Oktavio", STAT_SPELL_POWER,delta);
  		sleep(NHF_slep_singl);
  		NHF_Oktavio_SP_bonus = NHF_Oktavio_SP_bonus_temp;
  		sleep(3);
  		startThread(NHF_ShowFlyMessage,{NHF_Oktavio_textPath.."message.txt";p=delta}, temp_name ,8);
  		--startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ: "..delta),temp_name ,8);
  	end;
	end
end)

--NHF---------------пїЅпїЅпїЅпїЅпїЅпїЅ--------------------
NHF_Robert_index_battle_next = -1;
NHF_Robert_day_temp = 0;
function  error_NHF_Robert_F ()
	print("error:NHF_Robert_F");
end;

AddEvent(new_day_events, 'NHF_robert_new_day',
function(hero, day)
  if hero == 'Robert' and IsHeroAlive('Robert') then
  	local gold_plus = (GetHeroLevel("Robert")*NHF_Robert_percent);
  	if GetDate(ABSOLUTE_DAY) > NHF_Robert_day_temp then
  		NHF_Robert_day_temp = GetDate(ABSOLUTE_DAY);
  		NHF_ChangeResourceHero(GOLD, gold_plus, "Robert");
  		sleep(NHF_slep_singl);
  	end;
	end
end)

AddEvent(combat_results_events, 'NHF_robert_combat_result',
function(fight_id)
	errorHook(error_NHF_Robert_F);
	if (IsHeroAlive("Robert") == true) and (GetSavedCombatArmyHero(fight_id, 1)== "Robert") and (NHF_Robert_index_battle_next ~= fight_id) then
    NHF_Robert_index_battle_next = fight_id;
    local gold_plus = (GetHeroLevel("Robert")*NHF_Robert_percent);
		NHF_ChangeResourceHero(GOLD, gold_plus, "Robert");
		sleep(NHF_slep_singl);
	end;
...
--NHF--------------пїЅпїЅпїЅпїЅпїЅпїЅ--------------------------
NHF_Serhio_textPath = "/MapObjects/Haven/NewHeroes/Serhio/";
NHF_Serhio_index_battle_next = -1;
...
function(fight_id)
	...
	local temp_name = NHF_tempName_F("Serhio");
	if (GetSavedCombatArmyHero(fight_id, 1)== "Serhio") and (NHF_Serhio_index_battle_next ~= fight_id) then
	    NHF_Serhio_index_battle_next = fight_id;
	    local count_stacks = GetSavedCombatArmyCreaturesCount(fight_id,0);
	    local itogo_dead_count = 0;
	    ...
	    if itogo_dead_count > 0 then
			local prirost =  ceil((itogo_dead_count)*GetHeroLevel("Serhio"));
			ChangeHeroStat("Serhio", STAT_EXPERIENCE, prirost); -- пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ 0.85
			sleep(NHF_slep_singl);
			startThread(NHF_ShowFlyMessage, {NHF_Serhio_textPath.."NHF_Serhio_opyt.txt";count = prirost},temp_name , 5); 
			--ShowFlyingSign({"/MapObjects/Haven/NewHeroes/Serhio/NHF_Serhio_opyt.txt";count = prirost}, temp_name, GetObjectOwner(temp_name), 5);
	    end;
	end;
end)
...
--NHF---------------пїЅпїЅпїЅпїЅпїЅ-----------------------
combat_scripts_paths[length(combat_scripts_paths) + 1] = '/scripts/combat/NHF/Fiona.lua'

--NHF_E---------------пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ---------------------
NHF_GabriellNHF_textPath = "/MapObjects/Haven/NewHeroes/GabriellNHF/";
NHF_GabriellNHF_week_temp = -1;
function  error_NHF_GabriellNHF_F ()
	print("error:NHF_GabriellNHF_F");
end;
AddEvent(new_day_events, 'NHF_gabriell_new_day',
function(hero, day)
  if hero == 'GabriellNHF' and IsHeroAlive('GabriellNHF') then
    errorHook(error_NHF_GabriellNHF_F);
    if(mod(GetDate(ABSOLUTE_DAY),7) == 1) and (NHF_GabriellNHF_week_temp < GetDate(ABSOLUTE_DAY)) then
      NHF_GabriellNHF_week_temp =  GetDate(ABSOLUTE_DAY);
      local prirost_Airis = 0;
      local temp_name = NHF_tempName_F("GabriellNHF");
      if (IsHeroAlive("Ving") == true) then
         local temp_name_dop = NHF_tempName_F("Ving");
         if (GetPlayerTeam(GetObjectOwner(temp_name)) == GetPlayerTeam(GetObjectOwner(temp_name_dop)))  then
            prirost_Airis = GetHeroLevel("Ving");
         end;
      end;
      local prirost = GetHeroLevel("GabriellNHF") + prirost_Airis;
      if prirost > 0 then
         NHF_creatures_prirost_F("GabriellNHF",prirost,NHF_mass_creatures.Haven.L4);
      end;
    end
  end;
end)
...
--NHF_E-----------пїЅпїЅпїЅпїЅ----------------------------
NHF_KarloffNHF_day_temp = 0;
function  error_NHF_KarloffNHF_F ()
	print("error:NHF_KarloffNHF_F");
end;
AddEvent(new_day_events, 'NHF_karlo_new_day',
function(hero, day)
  if hero == 'KarloffNHF' and IsHeroAlive('KarloffNHF') then
    errorHook(error_NHF_KarloffNHF_F);
    if (mod(GetDate(ABSOLUTE_DAY),7) == 1) and (NHF_KarloffNHF_day_temp ~= GetDate(ABSOLUTE_DAY)) then
      NHF_KarloffNHF_day_temp = GetDate(ABSOLUTE_DAY);
      NHF_ChangeResourceHero(ORE, ceil(NHF_KarloffNHF_percent*GetHeroLevel("KarloffNHF")), "KarloffNHF");
  	sleep(NHF_slep_singl);
    end;
  end
end)

--NHF_E---------------пїЅпїЅпїЅпїЅпїЅпїЅ---------------------
NHF_Trevor_bonus = 0;
...
AddEvent(add_hero_events, 'NHF_trevor_thread_start',
function(hero)
  if hero == 'NHFTrevor' then
    startThread(
    function()
      while 1 do
        if IsHeroAlive('NHFTrevor') then
        	errorHook(error_NHF_Trevor_F);
        	local NHFTrevor_bonus_temp = 0;
        	for i, zakl in  NHF_Trevor_zakl_mass do
        		if KnowHeroSpell("NHFTrevor", zakl)  == true then
        			NHFTrevor_bonus_temp = NHFTrevor_bonus_temp + 1;
        		end;
        	end;
            if floor(NHFTrevor_bonus_temp/NHF_Trevor_percent) > floor(NHF_Trevor_bonus/NHF_Trevor_percent) then
        	   NHF_ChangeHeroStat("NHFTrevor", STAT_SPELL_POWER,floor(NHFTrevor_bonus_temp/NHF_Trevor_percent)-floor(NHF_Trevor_bonus/NHF_Trevor_percent));
        	   sleep(NHF_slep_singl);
        	   NHF_Trevor_bonus = NHFTrevor_bonus_temp;
        	end;
        end
      	sleep()
      end
...
--NHF_E---------------пїЅпїЅпїЅпїЅпїЅпїЅ--------------------
NHF_Fabris_index_battle_next = -2;
NHF_Fabris_perviy_flag = 0;
function  error_NHF_Fabris_F ()
	print("error:NHF_Fabris_F");
end;

AddEvent(add_hero_events, 'NHF_fabris_add_hero',
function(hero)
  if hero == 'Fabris' then
	 errorHook(error_NHF_Fabris_F);
   GiveHeroBattleBonus("Fabris", HERO_BATTLE_BONUS_INITIATIVE, NHF_Fabris_percent);
 end
end)

AddEvent(respawn_hero_events, 'NHF_fabris_respawn',
function(hero)
  if hero == 'Fabris' then
    GiveHeroBattleBonus("Fabris", HERO_BATTLE_BONUS_INITIATIVE, NHF_Fabris_percent);
  end
...
AddEvent(combat_results_events, 'NHF_fabris_combat_result',
function(fight_id)
   if (NHF_Fabris_index_battle_next ~= fight_id) then
     if (GetSavedCombatArmyHero(fight_id, 1)== "Fabris")  then
       NHF_Fabris_index_battle_next = fight_id;
	   GiveHeroBattleBonus("Fabris", HERO_BATTLE_BONUS_INITIATIVE, NHF_Fabris_percent);
     end;
     if (GetSavedCombatArmyHero(fight_id, 0)== "Fabris")  then
       NHF_Fabris_index_battle_next = fight_id;
     end;
   end;
   NHF_Fabris_index_battle_next = fight_id;
...
AddEvent(remove_hero_events, 'NHF_fabris_remove_hero',
function(hero)
  if hero == 'Fabris' then
	  NHF_Fabris_index_battle_next = -2;
	end
...
----------------пїЅпїЅпїЅпїЅ--------------------------
NHF_Calid_week_temp = -1;
function  error_NHF_Calid_F ()
	print("error:NHF_Calid_F");
end;

AddEvent(new_day_events, 'NHF_calid_new_day',
function(hero, day)
  if hero == 'Calid' and IsHeroAlive('Calid') then
  	errorHook(error_NHF_Calid_F);
  	if (ceil(GetDate(ABSOLUTE_DAY)/7) > 1) and (NHF_Calid_week_temp < (ceil(GetDate(ABSOLUTE_DAY)/7)-1)) then
  		NHF_Calid_week_temp =  (ceil(GetDate(ABSOLUTE_DAY)/7)-1);
  		local temp_name = NHF_tempName_F("Calid");
  		local prirost = ceil(GetHeroLevel("Calid")*NHF_Grol_percent);
  		if prirost > 0 then
  			NHF_creatures_prirost_F("Calid",prirost,NHF_mass_creatures.Inferno.L3);
  		end;
  	end;
	end
end)
...
----------------пїЅпїЅпїЅпїЅпїЅпїЅпїЅ------------------------
NHF_Oddrema_week_temp = -1;
function  error_NHF_Oddrema_F ()
	print("error:NHF_Oddrema_F");
end;
AddEvent(new_day_events, 'NHF_oddrema_new_day',
function(hero, day)
  if hero == 'Oddrema' and IsHeroAlive('Oddrema') then
  	errorHook(error_NHF_Oddrema_F);
  	if (ceil(GetDate(ABSOLUTE_DAY)/7) > 1) and (NHF_Oddrema_week_temp < (ceil(GetDate(ABSOLUTE_DAY)/7)-1)) then
  		NHF_Oddrema_week_temp =  (ceil(GetDate(ABSOLUTE_DAY)/7)-1);
  		local temp_name = NHF_tempName_F("Oddrema");
  		local prirost = ceil(GetHeroLevel("Oddrema")*NHF_Djezbet_percent);
  		if prirost > 0 then
  			NHF_creatures_prirost_F("Oddrema",prirost,NHF_mass_creatures.Inferno.L4);
  		end;
  	end;
	end
end)
...
----------------пїЅпїЅпїЅпїЅпїЅпїЅ--------------------
NHF_Jazaz_index_battle_next = -1;
NHF_Jazaz_flag =1;
NHF_Jazaz_lvl_temp = 0
function  error_NHF_Jazaz_F ()
	print("error:NHF_Jazaz_F");
end;

AddEvent(add_hero_events, 'NHF_jazaz_add_hero',
function(hero)
  if hero == 'Jazaz' then
   errorHook(error_NHF_Jazaz_F)
   if (NHF_Jazaz_flag == 0)then
     NHF_Jazaz_flag = 1;
     GiveHeroBattleBonus("Jazaz", HERO_BATTLE_BONUS_LUCK, floor( GetHeroLevel("Jazaz")/NHF_Jazaz_percent));
   end;
  end
end)

AddEvent(combat_results_events, 'NHF_jazaz_combat_result',
function(fight_id)
   if (NHF_Jazaz_index_battle_next ~= fight_id) then
     if (GetSavedCombatArmyHero(fight_id, 1)== "Jazaz") then
       NHF_Jazaz_index_battle_next = fight_id;
       GiveHeroBattleBonus("Jazaz", HERO_BATTLE_BONUS_LUCK, floor( GetHeroLevel("Jazaz")/NHF_Jazaz_percent));
     end;
     if (GetSavedCombatArmyHero(fight_id, 0)== "Jazaz")  then
       NHF_Jazaz_index_battle_next = fight_id;
       NHF_Jazaz_flag = 0;
     end;
   end;
...
AddEvent(level_up_events, 'NHF_jazaz_lvl_up',
function(hero)
  if hero == 'Jazaz' then
   if (GetHeroLevel("Jazaz")> NHF_Jazaz_lvl_temp) and (floor( GetHeroLevel("Jazaz")/NHF_Jazaz_percent)>floor(NHF_Jazaz_lvl_temp/NHF_Jazaz_percent))  then
     GiveHeroBattleBonus("Jazaz", HERO_BATTLE_BONUS_LUCK, 1);
     NHF_Jazaz_lvl_temp = GetHeroLevel("Jazaz");
   end;
  end
end)

AddEvent(remove_hero_events, 'NHF_jazaz_remove_hero',
function(hero)
  if hero == 'Jazaz' then
	 NHF_Jazaz_flag = 0;
	 NHF_Jazaz_index_battle_next = -2;
 end;	
end)
...
AddEvent(combat_results_events, 'NHF_aglion_combat_result',
function(fight_id)
errorHook(error_NHF_Aglion_F);
local prirost = 0;
local creatures = "";
	if (GetSavedCombatArmyHero(fight_id,1) == "Aglion") and   (fight_id~= NHF_Aglion_index_battle_next) then
		NHF_Aglion_index_battle_next = fight_id;
		local temp_name = NHF_tempName_F("Aglion");
		local prirost_mana = GetHeroCreatures("Aglion", CREATURE_FAMILIAR ) +  GetHeroCreatures("Aglion", CREATURE_IMP) + GetHeroCreatures("Aglion", CREATURE_QUASIT);		
		if  prirost_mana > 0 then			
			local mass_cherti = {CREATURE_FAMILIAR,CREATURE_IMP,CREATURE_QUASIT};	
			local count_stacks = 0;
			for i,creature in mass_cherti do
			    if count_stacks > 0 then
					sleep(3);
				end;
				prirost = ceil(GetHeroCreatures("Aglion", creature)*NHF_Aglion_persent);
				if prirost > 0 then
					NHF_creatures_prirost_F("Aglion",prirost,NHF_mass_creatures.Inferno.L1,creature);
					local  temp_knowledge = GetHeroStat("Aglion", STAT_KNOWLEDGE)*10;
					ChangeHeroStat("Aglion", STAT_KNOWLEDGE , temp_knowledge);
					sleep(NHF_slep_singl);
					local prirost_mana_fakt = prirost;
					ChangeHeroStat("Aglion", STAT_MANA_POINTS, prirost_mana_fakt);	
					sleep(NHF_slep_singl);					
					ChangeHeroStat("Aglion", STAT_KNOWLEDGE , -temp_knowledge);	
					sleep(NHF_slep_singl);
					sleep(1);
					startThread(NHF_ShowFlyMessage,{NHF_Aglion_textPath.."message.txt";p=prirost_mana_fakt}, temp_name,8);					
				end;
...
function  NHF_battle_BiaraNHF_F (name_gertva)
	errorHook(error_good_NHF_BiaraNHF_F);
	MakeHeroInteractWithObject("BiaraNHF",name_gertva);
	errorHook(error_good_NHF_BiaraNHF_F);
	return parse('');
end;

mccs_hero_custom['BiaraNHF'] =
function(heroBiaraNHF)
  errorHook(error_NHF_BiaraNHF_F);
  if heroBiaraNHF == "BiaraNHF" then
  local textPath = "/MapObjects/Inferno/NewHeroes/Biara/";
  local temp_name = NHF_tempName_F("BiaraNHF");
  local heroes = GetObjectNamesByType("HERO");
  local min_rasstoyanie_temp = 999;
  local min_rasstoyanie = 999;
  local name_gertva = "";
    for i,hero in heroes do
        if GetHeroTown(hero) == nil then
           if GetPlayerTeam(GetObjectOwner(hero)) ~=  GetPlayerTeam(GetObjectOwner(temp_name)) then ---???
           local x,y,z,x_BiaraNHF,y_BiaraNHF,z_BiaraNHF;
           x,y,z = GetObjectPosition(hero);
           x_BiaraNHF,y_BiaraNHF,z_BiaraNHF =  GetObjectPosition(temp_name);
              if z == z_BiaraNHF then
              min_rasstoyanie = sqrt(((x-x_BiaraNHF)*(x-x_BiaraNHF))+((y-y_BiaraNHF)*(y-y_BiaraNHF)));
                 if (min_rasstoyanie < min_rasstoyanie_temp) and (min_rasstoyanie <= NHF_Biara_percent ) then
                   min_rasstoyanie_temp = min_rasstoyanie;
                   name_gertva = hero;
                 end;
              end;
           end;
    ...
    if name_gertva ~= "" then
      local creature_mass = {};
      creature_mass[0], creature_mass[1], creature_mass[2], creature_mass[3], creature_mass[4], creature_mass[5], creature_mass[6] = GetHeroCreaturesTypes(name_gertva);
      local creature_mass_BiaraNHF = {};
      creature_mass_BiaraNHF[0], creature_mass_BiaraNHF[1], creature_mass_BiaraNHF[2], creature_mass_BiaraNHF[3], creature_mass_BiaraNHF[4], creature_mass_BiaraNHF[5], creature_mass_BiaraNHF[6] = GetHeroCreaturesTypes("BiaraNHF");
      local flag_spyor = 0;
      if (creature_mass[1] ~= 0) then
        for  j = 0,6 do
           for i = 0,6 do
             if(creature_mass[i]~=0)and(creature_mass_BiaraNHF[j]~=0) then			   
               if (NHF_tiers_creatures[creature_mass_BiaraNHF[j]] == NHF_tiers_creatures[creature_mass[i]])and(GetHeroCreatures("BiaraNHF",creature_mass_BiaraNHF[j])>=GetHeroCreatures(name_gertva,creature_mass[i])) then
               flag_spyor = 1;
			         local add_creatures = GetHeroCreatures(name_gertva,creature_mass[i]);
               AddHeroCreatures("BiaraNHF",creature_mass[i],add_creatures);
               RemoveHeroCreatures(name_gertva,creature_mass[i],add_creatures);
               local MP_BiaraNHF = GetHeroStat("BiaraNHF", STAT_MOVE_POINTS);
                 if MP_BiaraNHF > 2500  then
                    MP_BiaraNHF = 2500;
                 end;
               ChangeHeroStat("BiaraNHF", STAT_MOVE_POINTS, - MP_BiaraNHF);
      			   sleep(NHF_slep_singl);
      			   local temp_name_gertva = NHF_tempName_F(name_gertva);
               startThread(ShowFlyingSign, {'Text/Default/add_creature.txt'; name = GetCreatureName(creature_mass[i]), num = add_creatures}, temp_name, GetObjectOwner(temp_name), 7.0)
               startThread(ShowFlyingSign, {'Text/Default/remove_creature.txt'; name = GetCreatureName(creature_mass[i]), num = add_creatures}, temp_name_gertva, GetObjectOwner(temp_name_gertva), 7.0)
               --sleep(4)
               --MessageBoxForPlayers(GetObjectOwner(temp_name),textPath.."kraga_BiaraNHF.txt");
               break;
               end;
             end;
           end;
           if flag_spyor == 1 then
           break;
           end;
           if j == 6 then
		   startThread(NHF_ShowFlyMessage, textPath.."battle_BiaraNHF.txt",temp_name , 5);
		   sleep(3);
		   NHF_battle_BiaraNHF_F(name_gertva);
           --MessageBoxForPlayers(GetObjectOwner(temp_name),textPath.."battle_BiaraNHF.txt", "NHF_battle_BiaraNHF_F('"..name_gertva.."')");
           end;
        end;
      else
		startThread(NHF_ShowFlyMessage, textPath.."battle_BiaraNHF.txt",temp_name , 5);
...
AddEvent(add_hero_events, 'NHF_biara_add_hero',
function(hero)
  if hero == 'BiaraNHF' then
    startThread(
    function()
      while 1 do
        if IsHeroAlive('BiaraNHF') then
          errorHook(error_NHF_BiaraNHF_F);
          local temp_name = NHF_tempName_F("BiaraNHF");
          local heroes = GetObjectNamesByType("HERO");
          local min_rasstoyanie_temp = 999;
          ...
          for i,hero in heroes do
            if GetHeroTown(hero) == nil then
              if GetPlayerTeam(GetObjectOwner(hero)) ~=  GetPlayerTeam(GetObjectOwner(temp_name)) then
                ...
                if z == z_BiaraNHF then
                  min_rasstoyanie = sqrt(((x-x_BiaraNHF)*(x-x_BiaraNHF))+((y-y_BiaraNHF)*(y-y_BiaraNHF)));
          				if (min_rasstoyanie <= NHF_Biara_percent ) and (GetHeroStat("BiaraNHF", STAT_MOVE_POINTS) >= 2500) then
                    flag = 1
          				end;
                 end;
               end;
          ...
          if not flag then
            mccs_custom_ability.hero_custom_affected_heroes['BiaraNHF'][MCCS_CUSTOM_BIARA] = nil
          else
            mccs_custom_ability.hero_custom_affected_heroes['BiaraNHF'][MCCS_CUSTOM_BIARA] = 1
          end;
        end
        sleep()
      end
...
AddEvent(remove_hero_events, 'NHF_biara_remove_hero',
function(hero)
  if hero == 'BiaraNHF' then
    NHF_BiaraNHF_flag_blizost = 0;
	end
    ...
    while 1 do
      if IsHeroAlive('Valafar') then
        ...
        if (IsHeroAlive("Valafar") == true) then
      	local  x_Valafar,y_Valafar,z_Valafar = GetObjectPosition("Valafar"); -- 0.86 пїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅ
...
AddEvent(combat_results_events, 'NHF_zana_combat_result',
function(fight_id)
  errorHook(error_NHF_Zana_F);
  local flag_dobavka = 0;
  if (GetSavedCombatArmyHero(fight_id,1) == "Zana") and   (fight_id~= NHF_Zana_index_battle_next) then
    NHF_Zana_index_battle_next = fight_id;
    local temp_name = NHF_tempName_F("Zana");
    local count_stacks = GetSavedCombatArmyCreaturesCount(fight_id,0);
    for i_stacks = 0, count_stacks - 1 do
        local id_creatures, count_creatures, dead_count = GetSavedCombatArmyCreatureInfo(fight_id,0,i_stacks);
        if dead_count > 0 then
          for j,id_mass_creature in NHF_mass_creatures_Inferno do
              if  NHF_mass_creatures_Inferno[id_creatures] == 1 then
				  local persent = GetHeroStat("Zana", STAT_LUCK)*NHF_Zana_persent_loose*100;
				  if persent > 50 then persent = 50 end;
                  local prirost = floor((dead_count*persent)/100);
                  if prirost >= 1 then
                    flag_dobavka = 1;
                    AddHeroCreatures("Zana", id_creatures, prirost);
					local temp_prirost = prirost;
					prirost = 0;					
					startThread(NHF_ShowFlyMessage, NHF_nameCr_F(id_creatures, ": +"..temp_prirost),temp_name , (2+i_stacks)*2);
					sleep(2);
					break;
                  end;
              end;
    ...
    local count_stacks = GetSavedCombatArmyCreaturesCount(fight_id,1);
    for i_stacks = 0, count_stacks - 1 do
        local id_creatures, count_creatures, dead_count = GetSavedCombatArmyCreatureInfo(fight_id,1,i_stacks);
        if dead_count > 0 then
          for j,id_mass_creature in NHF_mass_creatures_Inferno do
              if  NHF_mass_creatures_Inferno[id_creatures] == 1 then			      
				  local persent = GetHeroStat("Zana", STAT_LUCK)*NHF_Zana_persent_win*100;
				  if persent > 50 then persent = 50 end;
                  local prirost = floor((dead_count*persent)/100);
                  if prirost >= 1 then
                    flag_dobavka = 1;
...
--NHF----------------пїЅпїЅпїЅпїЅпїЅ (пїЅпїЅпїЅпїЅпїЅ)---------------------
NHF_Ksana_textPath = "/MapObjects/Inferno/NewHeroes/Ksana/";
NHF_Ksana_lvl_temp = 1;
function  error_NHF_Ksana_F ()
	print("error:NHF_Ksana_F");
end;

AddEvent(level_up_events, 'NHF_ksana_level_up',
function(hero)
  if hero == 'Ksana' then
  if (ceil(GetHeroLevel("Ksana")/NHF_Ksana_percent) > ceil(NHF_Ksana_lvl_temp/NHF_Ksana_percent))  then
     local temp_name = NHF_tempName_F("Ksana");
     NHF_Ksana_lvl_temp = GetHeroLevel("Ksana");
  	 NHF_ChangeHeroStat("Ksana", STAT_SPELL_POWER,1);
  	 sleep(NHF_slep_singl);
     --ChangeHeroStat("Ksana", STAT_SPELL_POWER,1);
	 --startThread(NHF_ShowFlyMessage, NHF_Ksana_textPath.."NHF_Ksana_prirost.txt",temp_name, GetObjectOwner(temp_name),5);
  end;
  end
end)

--NHF----------------пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ-----------------------
NHF_Malustar_day_temp = 0;
function  error_NHF_Malustar_F ()
	print("error:NHF_Malustar_F");
end;

AddEvent(new_day_events, 'NHF_malustar_new_day',
function(hero, day)
  if hero == 'Malustar' and IsHeroAlive('Malustar') then
	errorHook(error_NHF_Malustar_F);
	if (mod(GetDate(ABSOLUTE_DAY),7) == 1) and (NHF_Malustar_day_temp ~= GetDate(ABSOLUTE_DAY)) then
		NHF_Malustar_day_temp = GetDate(ABSOLUTE_DAY);
		NHF_ChangeResourceHero(SULFUR, ceil(NHF_Malustar_percent*GetHeroStat("Malustar", STAT_KNOWLEDGE)), "Malustar");
		sleep(NHF_slep_singl);
	end;
	end
end)
...
AddEvent(add_hero_events, 'NHF_olotan_add_hero',
function(hero)
  if hero == 'Olotan' then
    startThread(
    function()
      while 1 do
        if IsHeroAlive('Olotan') then
          errorHook(error_NHF_Olotan_F);
          local count_creatures = 0;
          local Olotan_bonus_temp = 0;
        	for id,creature in NHF_mass_creatures_Inferno do
        	count_creatures = count_creatures + GetHeroCreatures("Olotan", id);
        	end;
        	Olotan_bonus_temp = floor(count_creatures/NHF_Olotan_percent);
        	if Olotan_bonus_temp ~= NHF_Olotan_bonus then
        		NHF_ChangeHeroStat("Olotan", STAT_DEFENCE,Olotan_bonus_temp-NHF_Olotan_bonus);
        		sleep(NHF_slep_singl);
        		NHF_Olotan_bonus = Olotan_bonus_temp;
        	end;
        end
        sleep()
      end
...
--NHF----------------пїЅпїЅпїЅпїЅпїЅ---------------------
NHF_Fagus_text_path = "/MapObjects/Inferno/NewHeroes/Fagus/bonus";
NHF_Fagus_day = 0;
NHF_Fagus_bonus = {0,0,0};
function  error_NHF_Fagus_F ()
	print("error:NHF_Fagus_F");
end;

AddEvent(new_day_events, 'NHF_fagus_new_day',
function(hero, day)
  if hero == 'Fagus' and IsHeroAlive('Fagus') then
  errorHook(error_NHF_Fagus_F);
  if  NHF_Fagus_day ~= day then
      NHF_Fagus_day = day;
      if NHF_Fagus_bonus[1] ~= 0 then
         ChangeHeroStat("Fagus",NHF_Fagus_bonus[1], -NHF_Fagus_bonus[2]);
		 sleep(NHF_slep_singl);
         if  NHF_Fagus_bonus[1] == 4 then 
			  local minus_mana = NHF_Fagus_bonus[3];
              if GetHeroStat("Fagus",STAT_MANA_POINTS) <= NHF_Fagus_bonus[3]   then
                 minus_mana = GetHeroStat("Fagus",STAT_MANA_POINTS);
              end;
              ChangeHeroStat("Fagus", STAT_MANA_POINTS, minus_mana);
			  sleep(NHF_slep_singl);
         end;
      end;
      local temp_name = NHF_tempName_F("Fagus");
      local statID_bonus  = random (4) + 1;
      local statDelta_bonus = ceil(GetHeroLevel("Fagus")/NHF_Fafus_percent);
      local K_PerkPrityagenie = 1;
      local prirost_mana = 0 ;
      local prirost = 0;
      ChangeHeroStat("Fagus",statID_bonus, statDelta_bonus);
	  sleep(NHF_slep_singl);
      if  statID_bonus == 4 then           
          if GetHeroSkillMastery("Fagus", PERK_INTELLIGENCE) == true then
             K_PerkPrityagenie = 1.5;
          end;		  
          prirost_mana =  ceil(K_PerkPrityagenie*10*statDelta_bonus);
		  ChangeHeroStat("Fagus",statID_bonus, 999);
		  sleep(NHF_slep_singl);
		  ChangeHeroStat("Fagus",STAT_MANA_POINTS, prirost_mana);
		  sleep(NHF_slep_singl);
		  ChangeHeroStat("Fagus",statID_bonus, - 999);
		  sleep(NHF_slep_singl);
      end;
      NHF_Fagus_bonus = {statID_bonus, statDelta_bonus,prirost_mana};
      ShowFlyingSign({NHF_Fagus_text_path..statID_bonus..".txt";prirost = statDelta_bonus, prirost_mana = prirost_mana},temp_name, GetObjectOwner(temp_name), 5);--???
	  end;
  end
end)

--NHF----------------пїЅпїЅпїЅпїЅпїЅпїЅ--------------------
NHF_Halfas_textPath = "/MapObjects/Inferno/NewHeroes/Halfas/";
NHF_Halfas_offence_temp = 0;
function  error_NHF_Halfas_F ()
	print("error:NHF_Halfas_F");
end;

AddEvent(add_hero_events, 'NHF_halfas_add_hero',
function(hero)
  if hero == 'Halfas' then
    startThread(
    function()
      while 1 do
        if IsHeroAlive('Halfas') then
        	errorHook(error_NHF_Halfas_F);
        	local temp_name = NHF_tempName_F("Halfas");
        	local Halfas_offence = 0;
        	for j = 1, 8 do
        	  if GetPlayerState(j) == 1 then
        		 if GetPlayerTeam(GetObjectOwner(temp_name)) ==  GetPlayerTeam(j) then
        			for i, hero in  GetPlayerHeroes(j)  do
        				if  IsHeroAlive(hero)  == true then
        					if GetHeroSkillMastery(hero, SKILL_GATING) > 0 then
        					   Halfas_offence = Halfas_offence + GetHeroLevel(hero);
        					end;
        				end;
        			end;
        		 end;
        	...
        	local Halfas_offence_bonus = ceil(Halfas_offence/NHF_Halfas_Percent);
        	if NHF_Halfas_offence_temp ~=  Halfas_offence_bonus then
        		ChangeHeroStat("Halfas", STAT_ATTACK,(Halfas_offence_bonus-NHF_Halfas_offence_temp));
        		sleep(NHF_slep_singl);
        		startThread(NHF_ShowFlyMessage,{NHF_Halfas_textPath.."message.txt";p=Halfas_offence_bonus}, temp_name,5);
        		--startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ: + "..Halfas_offence_bonus),temp_name , 5);
        		NHF_Halfas_offence_temp = Halfas_offence_bonus;
        	end;
        end
      	sleep()
      end
...
--NHF----------------пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ------------------
NHF_Elmandr_week_temp = 0;
function  error_NHF_Elmandr_F ()
	print("error:NHF_Elmandr_F");
end;

AddEvent(new_day_events, 'NHF_elmandr_new_day',
function(hero, day)
  if hero == 'Elmandr' and IsHeroAlive('Elmandr') then
  	errorHook(error_NHF_Elmandr_F);
  	if  NHF_Elmandr_week_temp ~= (ceil(GetDate(ABSOLUTE_DAY)/7)) then
  		NHF_Elmandr_week_temp =  (ceil(GetDate(ABSOLUTE_DAY)/7));
  		local temp_name = NHF_tempName_F("Elmandr");
  		local prirost = ceil(GetHeroLevel("Elmandr")*NHF_Elmandr_percent);
  		AddHeroCreatures("Elmandr", CREATURE_FIRE_ELEMENTAL,prirost );
  		startThread(NHF_ShowFlyMessage, NHF_nameCr_F(CREATURE_FIRE_ELEMENTAL, ": +"..prirost),temp_name ,5);
  	end;
	end
end)

--NHF_E-------------пїЅпїЅпїЅпїЅпїЅпїЅ--------------------
NHF_Kiril_index_battle_next = -2;
NHF_Kiril_perviy_flag = 0;
function  error_NHF_Kiril_F ()
	print("error:NHF_Kiril_F");
end;

AddEvent(add_hero_events, 'NHF_kiril_add_hero',
function(hero)
  if hero == 'Kiril' then
    GiveHeroBattleBonus("Kiril", HERO_BATTLE_BONUS_LUCK, NHF_Kiril_percent);
  end
end)

AddEvent(respawn_hero_events, 'NHF_kiril_respawn',
function(hero)
  if hero == 'Kiril' then
    GiveHeroBattleBonus("Kiril", HERO_BATTLE_BONUS_LUCK, NHF_Kiril_percent);
  end
...
function(fight_id)
  if (NHF_Kiril_index_battle_next ~= fight_id) then
		if (GetSavedCombatArmyHero(fight_id, 1)== "Kiril")  then
		   NHF_Kiril_index_battle_next = fight_id;
		   GiveHeroBattleBonus("Kiril", HERO_BATTLE_BONUS_LUCK, NHF_Kiril_percent);
		end;
		if (GetSavedCombatArmyHero(fight_id, 0)== "Kiril")  then
		   NHF_Kiril_index_battle_next = fight_id;
		end;
    end;
    NHF_Kiril_index_battle_next = fight_id;
...
AddEvent(remove_hero_events, 'NHF_kiril_remove_hero',
function(hero)
  if hero == 'Kiril' then
	  NHF_Kiril_index_battle_next = -2;
	end
...
--NHF_E---------------пїЅпїЅпїЅпїЅпїЅ--------------------
NHF_Rashka_week_temp = -1;
function  error_NHF_Rashka_F ()
	print("error:NHF_Rashka_F");
end;

AddEvent(new_day_events, 'NHF_rashka_new_day',
function(hero, day)
  if hero == 'Rashka' and IsHeroAlive('Rashka') then
  errorHook(error_NHF_Rashka_F);
  if  (mod(GetDate(ABSOLUTE_DAY),7) == 1) and (NHF_Rashka_week_temp < GetDate(ABSOLUTE_DAY)) then
      NHF_Rashka_week_temp =  GetDate(ABSOLUTE_DAY);
      local temp_name = NHF_tempName_F("Rashka");
      local prirost = 1+floor(GetHeroLevel("Rashka")*NHF_Rashka_percent);
      if prirost > 0 then
         NHF_creatures_prirost_F("Rashka",prirost,NHF_mass_creatures.Inferno.L6);
      end;
 end;
 end
end)
    ...
    function()
      while 1 do
        if IsHeroAlive('Nemor') then
          ...
          if  count_umertvie > 0 then --пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ 0.84
          	for i = 0,6 do
      ...
      while 1 do
        if IsHeroAlive('Abissid') then
          ...
          if NHF_Abissid_while_flag == 0 then
          			...
          			if (GetPlayerTeam(GetObjectOwner(temp_name))~= GetPlayerTeam(GetObjectOwner(hero)) or (GetHeroSkillMastery(hero, SKILL_NECROMANCY) < 1)) then
          			   ...
          			   if z == z_Abissid then
          				  ...
          				  if rasstoyanie <= NHF_Abissid_percent then
          					 ...
          					 --startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ"),hero,5);
          					 --NHF_ShowFlyMessage (NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ"),hero,5);
          					 --ShowFlyMessage(NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ"),hero , GetObjectOwner(hero),5);
          				  end;
          			   end;
          			end;
          		end;
            ...
            -- + пїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅ -0.79
            for i, hero in  GetObjectNamesByType("HERO") do
...
function(fight_id)
        ...
        if len(creatures_to_add) > 0 then
          for creature, count in creatures_to_add do
						...
						--startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ: +"..cr[2]), temp_name, 5);--frendly_counts												
					end;
				end;
		end;
end)
...
--NHF---------------пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ---------------------
...
local mass_spell_ShowFlyMessage = {}; 	
	if GetTownBuildingLevel(town, TOWN_BUILDING_MAGIC_GUILD)  == 5 and NHF_Heormel_gilds_flag < 5 then
		for i = 1, 12 do --пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ 0.85
	...
	if GetTownBuildingLevel(town, TOWN_BUILDING_MAGIC_GUILD)  == 4 and NHF_Heormel_gilds_flag < 4 then
        for i = 1, 10 do --пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ 0.85
          if KnowHeroSpell("Heormel", NHF_mass_spells_svet[i]) ~= true then
             TeachHeroSpell("Heormel", NHF_mass_spells_svet[i]);
			 mass_spell_ShowFlyMessage[NHF_mass_spells_svet[i]] = 1;
			 NHF_Heormel_Kn = NHF_Heormel_Kn + NHF_Heormel_Kn_percent;
	...
	if 	GetTownBuildingLevel(town, TOWN_BUILDING_MAGIC_GUILD)  == 3 and NHF_Heormel_gilds_flag < 3 then
        for i = 1, 7 do --пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ 0.85
          if KnowHeroSpell("Heormel", NHF_mass_spells_svet[i]) ~= true then
             TeachHeroSpell("Heormel", NHF_mass_spells_svet[i]);
			 mass_spell_ShowFlyMessage[NHF_mass_spells_svet[i]] = 1;
			 NHF_Heormel_Kn = NHF_Heormel_Kn + NHF_Heormel_Kn_percent;
	...
	if GetTownBuildingLevel(town, TOWN_BUILDING_MAGIC_GUILD)  == 2  and NHF_Heormel_gilds_flag < 2 then
        for i = 1, 5 do --пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ 0.85
          if KnowHeroSpell("Heormel", NHF_mass_spells_svet[i]) ~= true then
			 TeachHeroSpell("Heormel", NHF_mass_spells_svet[i]);
			 mass_spell_ShowFlyMessage[NHF_mass_spells_svet[i]] = 1;  
			 NHF_Heormel_Kn = NHF_Heormel_Kn + NHF_Heormel_Kn_percent;
...
-- Р°РєС‚РёРІР°С†РёСЏ СЃРїРµС†СѓРјРµРЅРёСЏ
function KiinaSpec_SummonCreatures(hero, mark)
  -- РЅР°Р№С‚Рё РјР°РєСЃРёРјР°Р»СЊРЅС‹Р№ СѓСЂРѕРІРµРЅСЊ СЃСѓС‰РµСЃС‚РІ РІ РіРѕСЂРѕРґР°С… РёРіСЂРѕРєР°, РєРѕС‚РѕСЂРѕРјСѓ РїСЂРёРЅР°РґР»РµР¶РёС‚ РљРёРёРЅР°
  ...
  -- РїРѕСЃС‚СЂРѕРµРє РЅРµС‚?
  if current_max_tier == -1 then
  -- РёРЅР°С‡Рµ
  else
  	-- РїСЂРѕРІРµСЂРёС‚СЊ, РµСЃС‚СЊ Р»Рё СЃСѓС‰РµСЃС‚РІР° СЌС‚РѕРіРѕ С‚РёСЂР° Сѓ РљРёРёРЅС‹
  	local kiina_max_tier_creatures = GetHeroCreaturesByTier(hero, TOWN_PRESERVE, current_max_tier)
  	local creature_to_add, count = 0, 0
  	-- РµСЃР»Рё РµСЃС‚СЊ, С‚Рѕ РґРѕР±Р°РІРёС‚СЊ СЃР»СѓС‡Р°Р№РЅРѕРµ РёР· РёРјРµСЋС‰РёС…СЃСЏ
  	if kiina_max_tier_creatures then
  	  creature_to_add = GetRandFromT(kiina_max_tier_creatures)
  	-- РёРЅР°С‡Рµ, СЃР»СѓС‡Р°Р№РЅРѕРµ РёР· СЌС‚РѕРіРѕ С‚РёСЂР°
  	else
...
function(fight_id)
	...
	if (GetSavedCombatArmyHero(fight_id, 1)== "Ula") and (NHF_Ula_index_battle_next ~= fight_id) then
		...
		if mana_plus > 0 then
			...
			--startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅ + "..mana_prirost),temp_name,5);
		end;
	else
		  ...
		  if  ((GetTownRace(town) == TOWN_DUNGEON)  and (GetObjectOwner(town) ==  GetObjectOwner(temp_name))) then
			if GetTownBuildingLevel(town, TOWN_BUILDING_DWELLING_5) ~= 0 then
			   ...
			   if prorost > 0 then 
				   ...
				   --startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅ: + "..prorost),town , 5);
			   end;
			end;
		  end;
		end;
...
function(hero, day)
  if hero == 'Ksuda' and IsHeroAlive('Ksuda') then
  	...
  	if (GetDate(ABSOLUTE_DAY) > NHF_Ksuda_date_temp) and ((GetObjectOwner(temp_name) == GetCurrentPlayer()) or (GetCurrentPlayer() == -1)) then	-- пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ 0.78, 0.81
  		...
  		if (random(100) < (GetHeroLevel("Ksuda")*3)) then
  			...
  			--startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ"),temp_name , 5);
  		end;
  	end;
  end
end)
...
function(hero, day)
  if hero == 'Gronulf' and IsHeroAlive('Gronulf') then
    ...
    if  (mod(GetDate(ABSOLUTE_DAY),7) == 1) and (NHF_Gronulf_week_temp < GetDate(ABSOLUTE_DAY)) then
    	...
    	--startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅпїЅ: +"..prirost),temp_name , 5);
    end;
  end
end)
    ...
    errorHook(error_NHF_Rikfrost_F);
  	local get_mana = NHF_Rikfrost_get_mana;
		...
		if count_pribavka_mana > 0 then
			...
			--startThread(NHF_ShowFlyMessage, NHF_rawtext_standart("пїЅпїЅпїЅпїЅ: +"..count_pribavka_mana), temp_name,5);
		end;
	end;
...
-- С„СѓРЅРєС†РёСЏ, РїСЂРѕРІРµСЂСЏСЋС‰Р°СЏ, СЃРєРѕР»СЊРєРѕ РіРµСЂРѕРµРІ РЅР°С…РѕРґРёС‚СЃСЏ РІ РїРѕРґС…РѕРґСЏС‰РµРј РґР»СЏ РІРѕСЂРѕРІСЃС‚РІР° СЂР°РґРёСѓСЃРµ, Рё РµСЃР»Рё РёС… Р±РѕР»СЊС€Рµ РѕРґРЅРѕРіРѕ, С‚Рѕ Р·Р°РїСѓСЃРєР°РµС‚ РґРёР°Р»РѕРі РІС‹Р±РѕСЂР°, РёРЅР°С‡Рµ - СЃСЂР°Р·Сѓ РІРѕСЂСѓРµС‚
mccs_hero_custom['Avran'] =
function(hero)
  local heroes_to_steal = AvranaSpec_CheckHeroesNear(hero)
  -- РіРµСЂРѕР№ РІСЃРµРіРѕ РѕРґРёРЅ?
  if length(heroes_to_steal) == 1 then
		-- СЃСЂР°Р·Сѓ РІРѕСЂСѓРµРј
		startThread(AvranaSpec_StealArts, 'Avran', heroes_to_steal[1])
  else
		-- РёРЅР°С‡Рµ, СЃРѕС…СЂР°РЅРёС‚СЊ С‚Р°Р±Р»РёС†Сѓ РіРµСЂРѕРµРІ РґР»СЏ РєСЂР°Р¶Рё РіР»РѕР±Р°Р»СЊРЅРѕ, С‡С‚РѕР±С‹ РёРЅРёС†РёР°Р»РёР·РёСЂРѕРІР°С‚СЊ РґРёР°Р»РѕРі
		avrana_spec.curr_heroes_to_steal = heroes_to_steal
		-- Р·Р°РїСѓСЃС‚РёС‚СЊ РґРёР°Р»РѕРі РІС‹Р±РѕСЂР° РіРµСЂРѕРµРІ
  ...
  while max_tier_to_steal > 0 do
  	for i = 1, arts_count_to_steal[max_tier_to_steal] do
  	  ...
  	  if arts_to_steal then
    		-- СЃРѕРєСЂР°С‚РёС‚СЊ РІРґРІРѕРµ С‡РёСЃР»Рѕ РїРѕ РѕСЃС‚Р°РІС€РёРјСЃСЏ С‚РёСЂР°Рј, РіР»Р°РІРЅС‹Рј РѕР±СЂР°Р·РѕРј РґР»СЏ СЃРёС‚СѓР°С†РёРё, РµСЃР»Рё РЅРµС‚ СЂРµР»РёРєРѕРІ Рё РІСЃРµРіРѕ 1 РјР°Р¶РѕСЂ(РґРѕР»Р¶РЅРѕ РїРѕР»СѓС‡РёС‚СЊСЃСЏ 0-1-2)
